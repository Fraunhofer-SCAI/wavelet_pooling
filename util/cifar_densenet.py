...
class DenseNet(nn.Module):
    def __init__(self, block, nblocks, growth_rate=12,
        ...
        self.trans1 = Transition(num_planes, out_planes, pool=pool)
        num_planes = out_planes

        self.dense2 = self._make_dense_layers(block, num_planes, nblocks[1])
        num_planes += nblocks[1]*growth_rate
        out_planes = int(math.floor(num_planes*reduction))
        self.trans2 = Transition(num_planes, out_planes, pool=pool)
        num_planes = out_planes

        self.dense3 = self._make_dense_layers(block, num_planes, nblocks[2])
        num_planes += nblocks[2]*growth_rate
        out_planes = int(math.floor(num_planes*reduction))
        self.trans3 = Transition(num_planes, out_planes, pool=pool)
        num_planes = out_planes
...
def densenet_cifar(pool):
    return DenseNet(Bottleneck, [6, 12, 24, 16], growth_rate=12, pool=pool)
